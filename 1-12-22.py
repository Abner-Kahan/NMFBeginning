...
def nmf2TesterMixB():
    ...
    IR1 =gaussian_broadening(fetchIr('MeOHSample.txt',FileSelection1),25)
    IrPlotter(IR1,"MeOH Spectra")
   # print(IR1.shape)
    ...
    model = NMF(n_components=4, max_iter=4000, tol= 1*10**-10, solver= 'mu', beta_loss= 'kullback-leibler' )
...
def nmf2TesterMixB():
    ...
    FileSelection = random.randrange(1,12)
    print(f'The first selection is {FileSelection}.')

    #print(f'The expected fractions are  {fraction1:.3}, {fraction2:.3}.')
    
    #Creating Two Spectra
    IR0 =gaussian_broadening(fetchIr('MeOHSample.txt',FileSelection),25)
    IrPlotter(IR0,"MeOH Spectra")


    IRF= np.transpose(IR0)
    IRF = IRF.reshape(1,-1)
    ...
    print("first product", product.shape)
    #IrPlotter(IR0, product[0,:],"First Calculated Spectra", ['Original Ir', 'NMF Generated IR'],True)
    #IrPlotter(IR0 ,product[:,1],"Second Calculated Spectra", ['Original Ir', 'NMF Generated IR'], True)
    #IrPlotter(IR0 ,product[:,3],"Second Calculated Spectra", ['Original Ir', 'NMF Generated IR'], True)  
    #IrPlotter(IR0 ,product[:,4],"Second Calculated Spectra", ['Original Ir', 'NMF Generated IR'], True)  
